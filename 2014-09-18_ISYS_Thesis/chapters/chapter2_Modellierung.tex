...
T_\mathrm{Cl,ss} = 2\,N_{\mathrm{Disc},k}\,r_{\mathrm{m},k}\,F_\mathrm{Stribeck}.
\end{align}
Die Reibwerte $\mu_\mathrm{S}$ und $\mu_\mathrm{C}$ werden aus Kennfeldern in Abhängigkeit der des Drucks an den Scheiben $p_\mathrm{N}$, der Öltemperatur $T_\mathrm{Oil}$ und $\Delta \omega$ gewonnen. Die Normalkraft $F_\mathrm{N}$ ergibt sich aus $p_\mathrm{N}$ und der Fläche der Scheiben $A_\mathrm{Disc}$ und wird vorgegeben. 